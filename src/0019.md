...
// 只有当组件所处的树中没有匹配到 Provider 时，其 defaultValue 参数才会生效。-- themes.light
    ...
    <ThemeContext.Provider value={state}>
      <Toolbar
        themeClick={() => {
          ...
          setState((state) =>
            state.background === themes.dark.background
              ? themes.light
              : themes.dark
          );
        }}
      />
      {/* 虽然conosle function里面没有消费context，但是因为被包裹进去了，所以每次都会更新 */}
      <Console />
...
export default function Toolbar(props) {
  return (
    <div>
      <ThemedButton onClick={props.themeClick} />
    </div>
  );
}
```
